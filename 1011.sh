...
python ./TinyBERT/task_distill.py --teacher_model $BERT_BASE_DIR --student_model $GENERAL_TINYBERT_DIR --data_dir $TASK_DIR  --task_name $TASK_NAME --output_dir $MODEL_STG1_DIR --max_seq_length 128 --train_batch_size 32  --learning_rate 5e-6 --num_train_epochs 6 --do_lower_case --weight_decay 1e-5 --aug_train >$LOG
...
python ./TinyBERT/task_distill.py --pred_distill --teacher_model $BERT_BASE_DIR --student_model $MODEL_STG1_DIR --data_dir $TASK_DIR --task_name $TASK_NAME --output_dir $MODEL_STG2_DIR --do_lower_case --learning_rate 3e-6  --num_train_epochs  6 --eval_step 2000 --max_seq_length 128 --train_batch_size 32 --weight_decay 1e-5 --aug_train >$LOG
