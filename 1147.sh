...
python ./TinyBERT/task_distill.py --teacher_model $BERT_BASE_DIR --student_model $GENERAL_TINYBERT_DIR --data_dir $TASK_DIR  --task_name $TASK_NAME --output_dir $MODEL_STG1_DIR --max_seq_length 128 --train_batch_size 32 --num_train_epochs 3 --learning_rate 1e-4 --weight_decay 0.01 --do_lower_case --aug_train >$LOG
...
python ./TinyBERT/task_distill.py --pred_distill --teacher_model $BERT_BASE_DIR --student_model $MODEL_STG1_DIR --data_dir $TASK_DIR --task_name $TASK_NAME --output_dir $MODEL_STG2_DIR --do_lower_case --learning_rate 3e-5  --num_train_epochs  4 --eval_step 2000 --max_seq_length 128 --train_batch_size 32 --aug_train >$LOG
